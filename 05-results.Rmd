...
In our analysis of the aspiring data scientists in our class by gender, we wanted to identify if any differences existed in our professional and academic interests. In our survey, we asked participants to select the industry they would most like to work in as well as the field they most want to study. Our first heatmap analyzes the relationship between industry and academic background faceted on gender. The second heatmap explores a similar question on one’s preference in a data science research topic.



```{r message=FALSE}
#setwd("C:/Users/Shreyas Jadhav/Documents")
...
p9 <- ggplot(mydata_new_1, aes(x = Sex , y = Industry)) +
  ...
  coord_fixed() + theme_heat + 
      ...
      # formatting
    labs(title = "How does sex affect \n choice of industry?") +
    xlab("Sex") + ylab("Choice of Industry") + 
...
p10 <- ggplot(mydata_new_2, aes(x = Sex , y = DS_Subject)) +
  ...
  coord_fixed() + theme_heat+
    ...
    xlab("Sex") + ylab("Favourite research topics")+ 
...
Data Science, like many other STEM fields, is overwhelmingly male. This begs the question: Is data science becoming an all-boys club? We asked survey participants to select at most five people with whom they interact the most within our Exploratory Data Analysis and Visualization class. Using this data, we were able to see the explore the patterns in the social interactions among our classmates.
...
Across background big tech firms reigns supreme as the most popular industry for post-graduation. Perhaps, a majority of the students here are pursuing a Master’s degree so that they can break into the tech industry. However, curiously, survey participants with a Mathematics/Statistics background appear to have a higher affinity for the opportunities in the financial sector. Additionally, of those who responded, only people with a history in computing reported an interest in healthcare opportunities.
...
##### Stacked Bar Chart (Background breakdown by prior institution):
...
p4<- HH::likert(Measure ~ "Strongly Disagree" + "Disagree" + "Neutral" + "Agree" + "Strongly Agree",  
           ...
           main = "Responses to :'I love New York'",
